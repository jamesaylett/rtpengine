...
static struct rtp_payload_type *rtp_payload_type_dup(const struct rtp_payload_type *pt);
		...
		struct call_media *sink, GQueue *sink_list)
{
	...
	for (GList *l = sink->codecs_send.codec_prefs.head; l; l = l->next) {
		...
		if (pt->for_transcoding) {
			// codec is explicitly marked for transcoding
...
static void __single_codec(struct call_media *media, const struct sdp_ng_flags *flags) {
	if (!flags || flags->opmode != OP_ANSWER || !flags->single_codec)
		return;
	int have_codec = 0;
	for (GList *l = media->codecs_recv.codec_prefs.head; l;) {
		struct rtp_payload_type *pt = l->data;
		...
		ilogs(codec, LOG_DEBUG, "Removing codec '" STR_FORMAT "' due to 'single codec' flag",
				STR_FMT(&pt->encoding_with_params));
		codec_touched(&media->codecs_recv, pt);
...
//	__make_transcoder(receiver->dtmf_injector, pref_dest_codec, output_transcoders, dtmf_payload_type, 0, -1);
...
void codec_handlers_update(struct call_media *receiver, struct call_media *sink,
		const struct sdp_ng_flags *flags, const struct stream_params *sp)
{
	ilogs(codec, LOG_DEBUG, "Setting up codec handlers");
	...
	struct rtp_payload_type *pref_dest_codec;
...
//		pcm_dtmf_detect = 1;
	...
	// for answers, re-create the list of codecs based on the reverse path from the offer
	AUTO_CLEANUP_INIT(GQueue accepted_codecs_prefs, g_queue_clear, G_QUEUE_INIT);
...
//		}
		...
		if (flags && flags->opmode == OP_ANSWER) {
			// look up the reverse side codec and see what it transcodes to
//			struct rtp_payload_type *recv_pt = g_hash_table_lookup(receiver->codecs_recv,
//					GINT_TO_POINTER(pt->payload_type));
			struct codec_handler *sink_h = codec_handler_get(sink, pt->payload_type);
			if (sink_h && sink_h->source_pt.payload_type == pt->payload_type
					&& sink_h->dest_pt.payload_type != -1)
			{
				ilogs(internals, LOG_DEBUG, "reverse dest codec is " STR_FORMAT,
						STR_FMT(&sink_h->dest_pt.encoding_with_full_params));
				sink_pt = &sink_h->dest_pt;
			}

///			// go through the codecs that the sink can receive and find the first that
///			// sends/transcodes this receive codec
///			for (GList *k = sink->codecs_prefs_recv.head; k; k = k->next) {
///				struct rtp_payload_type *recv_pt = k->data;
///				struct codec_handler *recv_h = codec_handler_get(sink, recv_pt->payload_type);
///				ilogs(internals, LOG_DEBUG, "checking sink codec " STR_FORMAT " %i/%i/%i",
///						STR_FMT(&recv_pt->encoding), pt->payload_type,
///						recv_pt->payload_type,
///						recv_h->dest_pt.payload_type);
///				if (recv_h && recv_h->dest_pt.payload_type == pt->payload_type) {
///					sink_pt = recv_pt;
///					ilogs(internals, LOG_DEBUG, "accept sink codec is " STR_FORMAT,
///							STR_FMT(&sink_pt->encoding));
///					break;
///				}
///			}
		}
		...
		if (!recv_dtmf_pt && sink_dtmf_pt && sink_dtmf_pt->for_transcoding) {
			pcm_dtmf_detect = 1;
...
next:
		if (flags && flags->opmode == OP_ANSWER && sink_pt)
			g_queue_push_tail(&accepted_codecs_prefs, GINT_TO_POINTER(sink_pt->payload_type));
...
//
//		if (!flags || flags->opmode != OP_ANSWER) {
//			// remove all unsupported codecs from the list
...
//		}
//		else {
//			// answer: rebuild the list of codecs based on what we will transcode to.
//			// steal the existing objects and re-initialise:
//			GQueue old_prefs_recv = receiver->codecs_prefs_recv;
//			GHashTable *old_recv = receiver->codecs_recv;
//
//			g_queue_init(&receiver->codecs_prefs_recv);
//			receiver->codecs_recv = g_hash_table_new(g_direct_hash, g_direct_equal);
//			g_hash_table_remove_all(receiver->codec_names_recv);
//
//			while (accepted_codecs_prefs.length) {
//				int ptnum = GPOINTER_TO_INT(g_queue_pop_head(&accepted_codecs_prefs));
//				ilogs(internals, LOG_DEBUG, "checking recv codec %i", ptnum);
//				struct rtp_payload_type *pt = g_hash_table_lookup(receiver->codecs_send,
//						GINT_TO_POINTER(ptnum));
//				if (!pt)
//					continue;
//				// avoid duplicates
//				if (g_hash_table_lookup(receiver->codecs_recv, GINT_TO_POINTER(ptnum)))
//					continue;
//				__rtp_payload_type_add_recv(receiver, __rtp_payload_type_copy(pt), 1);
//				// to eliminate duplicates:
//				//g_hash_table_remove(old_recv, GINT_TO_POINTER(ptnum));
//			}
//
//			g_hash_table_destroy(old_recv);
//			// finally clean up the PTs there aren't used
//			while (old_prefs_recv.length) {
//				struct rtp_payload_type *pt = g_queue_pop_head(&old_prefs_recv);
//				if (g_hash_table_lookup(receiver->codecs_recv, GINT_TO_POINTER(pt->payload_type)))
//					continue;
//				payload_type_free(pt);
//			}
//		}
...
//			}
	...
	// XXX XXX accept routine
//	if (flags && flags->opmode == OP_ANSWER) {
//		if (MEDIA_ISSET(sink, TRANSCODE) || MEDIA_ISSET(receiver, TRANSCODE))
//			__reorder_transcode_codecs(receiver, sink, flags, 0);
//	}
...
static int __handler_func_sequencer(struct media_packet *mp, struct transcode_packet *packet)
{
	...
	ilog(LOG_DEBUG, "XX  XXXXXXXXXXXXXXXX " STR_FORMAT, STR_FMT(&ch->handler->source_pt.encoding_with_params));
...
//				__make_transcoder(handler, &existing_handler->dest_pt, NULL, existing_handler->dtmf_payload_type, 0, existing_handler->cn_payload_type);
//			}
//
//			__rtp_payload_type_add_recv(media, pt, 1);
...
void codec_store_answer(struct codec_store *dst, struct codec_store *src, struct sdp_ng_flags *flags) {
	...
	for (GList *l = src->codec_prefs.head; l; l = l->next) {
		...
		if (pt->codec_def && pt->codec_def->supplemental) {
			is_supp = 1;
			ilog(LOG_DEBUG, "XXXXXX supp %p %p %p %i %i %i %i",
					dst->tracker, src->tracker, orig_dst.tracker,
					pt->codec_def->supplemental,
					is_codec_touched(dst, pt), is_codec_touched(src, pt),
					is_codec_touched(&orig_dst, pt));
			if (pt->for_transcoding)
				continue;
